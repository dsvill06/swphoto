...
const MediaItem = ({ item, className, onClick }: { item: MediaItemType, className?: string, onClick?: () => void }) => {
    ...
    return (
        <Image
            ...
            className={`${className} object-cover object-top cursor-pointer h-full `} // Style the image
                ...
                <div className="h-full flex flex-col">
                    <div className="flex-1 p-2 sm:p-3 md:p-4 flex items-center justify-center bg-gray-50/50">
                        <AnimatePresence mode="wait">
                            <motion.div
                                key={selectedItem.id}
                                className="relative w-full aspect-[25/20] max-w-[95%] sm:max-w-[85%] md:max-w-3xl 
                                         h-auto max-h-[80vh] rounded-lg overflow-hidden shadow-md object-contain"
                                ...
                                onClick={onClose}
                            >
                                <MediaItem item={selectedItem} className="w-full h-full object-scale-down bg-gray-900/20" onClick={onClose} />
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems, title, description }) => {
    ...
    return (
        <div className="container mx-auto px-4 py-8 w-full bg-grandient">
            ...
            <AnimatePresence mode="wait">
                {selectedItem ? (
                    ...
                    <motion.div
                        className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 gap-3 auto-rows-[minmax(120px,1fr)]"
                        initial="hidden"
                        animate="visible"
                        exit="hidden"
                        variants={{
